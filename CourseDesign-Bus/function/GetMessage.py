...
def NearestStation(pos):
    ...
    return Station[position], Time[position], temp_list
...
def GetDownTime(up_time, station):
    ...
    return Time[station],tempA,tempB
    ...
    for item in Bus:
        for i in Bus[item]:
            if station == i:
                for it in Bus[item]:
                    print(it, end=" ")
                    temp_list_route.append(it)
    pass
    return temp_list_time, temp_list_route
...
if __name__ == "__main__":
    ...
    time = input("时间：")
    station = input("站点：")
    GetDownTime(time, station)
    # GetStationMessage(station)
    pass
